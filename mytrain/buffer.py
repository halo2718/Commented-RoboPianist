...
class ReplayBuffer:
    def __init__(self, capacity, state_size, action_size, device, seed):
        self.device = device
        self.state = torch.zeros(capacity, state_size, dtype=torch.float).contiguous()
        self.action = torch.zeros(capacity, action_size, dtype=torch.float).contiguous()
        self.reward = torch.zeros(capacity, dtype=torch.float).contiguous()
        self.next_state = torch.zeros(capacity, state_size, dtype=torch.float).contiguous()
        self.done = torch.zeros(capacity, dtype=torch.int).contiguous()
    ...
    def add(self, transition):
        ...
        self.state[self.idx] = torch.from_numpy(state).to(self.device)
        self.action[self.idx] = torch.from_numpy(np.array([action])).to(self.device)
        self.reward[self.idx] = torch.from_numpy(np.array([reward])).to(self.device)
        self.next_state[self.idx] = torch.from_numpy(next_state).to(self.device)
        self.done[self.idx] = torch.from_numpy(np.array([done])).to(self.device)
    ...
    def sample(self, batch_size):
        ...
        state = self.state[sample_idxs]
        action = self.action[sample_idxs]
        reward = self.reward[sample_idxs]
        next_state = self.next_state[sample_idxs]
        done = self.done[sample_idxs]
