...
class TD3Agent(DDPGAgent):
    ...
    def get_Qs(self, state, action, reward, next_state, done) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            noise = torch.clamp(torch.randn_like(next_action, device=next_action.device) * self.policy_noise, -self.noise_clip, self.noise_clip)
            next_action = torch.clamp(next_action + noise, self.actor_net.action_space.low, self.actor_net.action_space.high)
            Q_target = reward + (1 - done) * self.gamma * torch.min(next_Q, next_Q2)
