...
class SACAgent(TD3Agent):
    ...
    def get_Qs(self, state, action, reward, next_state, done) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            next_action, log_prob_next = self.actor_net.evaluate(next_state)
            # print(next_state.shape, next_action.shape, log_prob_next.shape, reward.shape, log_prob_next.shape)
            next_Q = self.critic_target(next_state, next_action)
            next_Q2 = self.critic_target_2(next_state, next_action)
            Q_target = reward + self.gamma * (1 - done) * (torch.min(next_Q, next_Q2) - self.log_alpha.exp() * log_prob_next)
        ############################
    ...
    def get_actor_loss(self, state) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Q = self.critic_net(state, action)
        actor_loss = (self.log_alpha.detach().exp() * log_prob - Q).mean()
