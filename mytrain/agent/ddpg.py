...
class DDPGAgent:
    ...
    def get_Qs(self, state, action, reward, next_state, done) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Q_target = reward + (1 - done) * self.gamma * next_Q.squeeze()
    ...
    def get_action(self, state, sample=False):
        ...
        if sample:
            epsilon = self.eps_schedule(self.train_step)
            noise = torch.normal(0, epsilon, size=action.shape, device=state.device)
            action = torch.clamp(action + noise, min=self.actor_net.action_space.low, max=self.actor_net.action_space.high)
