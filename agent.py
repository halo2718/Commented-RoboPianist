...
class SacAgent:
    ...
    def __init__(self, train_env, eval_env, log_dir, num_steps=3000000, batch_size=256,
                 lr=0.0003, hidden_units=[256, 256, 256], memory_size=1e6,
        ...
        # get the shape of observation and action space
        self.action_shape = train_env.action_spec().shape
        self.observ_shape = self.get_observations(train_env.reset()).shape
        ...
        if per:
            ...
            self.memory = PrioritizedMemory(
                memory_size, self.observ_shape,
                self.action_shape, self.device, gamma, multi_step,
    ...
    def act(self, state):
        if self.start_steps > self.steps:
            action = (np.random.rand(self.action_shape[0])-0.5)*2
        else:
    ...
    def train_episode(self):
        ...
        state = self.get_observations(timestep)
    ...
    def evaluate(self):
        ...
        for i in range(episodes):
            state = self.get_observations(self.eval_env.reset())
            episode_reward = 0.
            done = False
            while not done:
